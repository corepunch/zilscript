...
-- Ensure GLOBAL/SETG/GETG are available for ZIL global variable access.
-- The full versions (with mem-based storage) are defined in bootstrap.lua;
-- these minimal stubs allow ZIL modules to be loaded without bootstrap.
if not _G.GLOBAL then _G.GLOBAL = function(name, val) _G[name] = val end end
if not _G.SETG then _G.SETG = function(name, val) _G[name] = val; return val end end
if not _G.GETG then _G.GETG = function(addr) return addr end end
